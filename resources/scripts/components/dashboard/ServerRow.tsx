                ...
                <p css={tw`text-sm text-neutral-400 ml-2`}>
                    {
                        server.allocations.filter(alloc => alloc.isDefault).map(allocation => (
                            <React.Fragment key={allocation.ip + allocation.port.toString()}>
                                <i className="bi bi-reception-4 icn"></i> {allocation.alias || allocation.ip}:{allocation.port}
                            </React.Fragment>
                        ))
                    }
                </p>
                    ...
                    <React.Fragment>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                                CPU: {stats.cpuUsagePercent.toFixed(2)} % of {cpuLimit}
                            </div>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                                MEM: {bytesToHuman(stats.memoryUsageInBytes)} of {memoryLimit}
                            </div>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                                DISK: {bytesToHuman(stats.diskUsageInBytes)}
                            </div>
                        </div>
                    </React.Fragment>
                }
